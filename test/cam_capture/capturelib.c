...
#ifdef CUSTOM_CHANGE
int dump_diff_image(void)
{
    int i, newi;
    int size = HRES*VRES*PIXEL_SIZE;

    int cur_head_idx, next_head_idx = 0;

    cur_head_idx = (frame_buffer.head_idx) % frame_buffer.ring_size;
    next_head_idx = (frame_buffer.head_idx + 1) % frame_buffer.ring_size;

    unsigned char *first_frame_ptr  = (unsigned char *) &frame_buffer[cur_head_idx].save_frame.frame[0];
    unsigned char *second_frame_ptr = (unsigned char *) &frame_buffer[next_head_idx].save_frame.frame[0];
    
    unsigned char      diff_frame[HRES*VRES*PIXEL_SIZE];
    unsigned char      first_gray_frame[HRES*VRES*PIXEL_SIZE];
    unsigned char      second_gray_frame[HRES*VRES*PIXEL_SIZE];

    process_framecnt++;
    printf("process frame %d: ", process_framecnt);

    for(i=0, newi=0; i < size; i+2, newi++)
    {
        if (first_frame_ptr[i] > second_frame_ptr[i+1]) { 
            diff_frame[newi]=first_frame_ptr[i] - second_frame_ptr[i];
        } else {
            diff_frame[newi]=second_frame_ptr[i] - first_frame_ptr[i];
        }
    }

    for(i=0, newi=0; i<size; i=i+4, newi=newi+2)
    {
            first_gray_frame[newi] = first_frame_ptr[i];
            first_gray_frame[newi+1] = first_frame_ptr[i+2];

            second_gray_frame[newi] = second_frame_ptr[i];
            second_gray_frame[newi+1] = second_frame_ptr[i+2];
    }
   
    save_image((*char) &first_gray_frame[0], size, &time_now);
    save_image((*char)&second_gray_frame[0], size, &time_now);
    save_image((*char)&first_gray_frame[0], size, &time_now);
    
    if (process_framecnt >= FRAMES_TO_ACQUIRE) {
        abort(void);
    }
...
int seq_frame_read(void)
{
    ...
    /* return error in buffer overflow case */
    if (frame_buffer.save_frame[frame_buffer.tail_idx].q_empty != TRUE) {
        printf("\n FRAME_BUFFER_FULL: Resource unavailable\n");
        return -1; // FIXME: return code and error message
    }

    // save off copy of image with time-stamp here
    //printf("memcpy to %p from %p for %d bytes\n", (void *)&(frame_buffer.save_frame[frame_buffer.tail_idx].frame[0]), buffers[frame_buf.index].start, frame_buf.bytesused);
    //syslog(LOG_CRIT, "memcpy to %p from %p for %d bytes\n", (void *)&(frame_buffer.save_frame[frame_buffer.tail_idx].frame[0]), buffers[frame_buf.index].start, frame_buf.bytesused);
    memcpy((void *)&(frame_buffer.save_frame[frame_buffer.tail_idx].frame[0]), buffers[frame_buf.index].start, frame_buf.bytesused);

    frame_buffer.save_frame[frame_buffer.tail_idx].q_empty = FALSE;
    frame_buffer.tail_idx = (frame_buffer.tail_idx + 1) % frame_buffer.ring_size;
    frame_buffer.count++;
    ...
    if(read_framecnt > 0)
    {	
        //printf("read_framecnt=%d, rb.tail=%d, rb.head=%d, rb.count=%d at %lf and %lf FPS", read_framecnt, frame_buffer.tail_idx, frame_buffer.head_idx, frame_buffer.count, (fnow-fstart), (double)(read_framecnt) / (fnow-fstart));

        //syslog(LOG_CRIT, "read_framecnt=%d, rb.tail=%d, rb.head=%d, rb.count=%d at %lf and %lf FPS", read_framecnt, frame_buffer.tail_idx, frame_buffer.head_idx, frame_buffer.count, (fnow-fstart), (double)(read_framecnt) / (fnow-fstart));
...
int seq_frame_process(void)
{
    ...
    printf("processing rb.tail=%d, rb.head=%d, rb.count=%d\n", frame_buffer.tail_idx, frame_buffer.head_idx, frame_buffer.count);
    ...
    for (;;) {
        ...
        if ((frame_buffer.save_frame[cur_head_idx].q_empty == TRUE) || (frame_buffer.save_frame[next_head_idx].q_empty == TRUE)) {
            printf("\nFRAME_PROCESS: Frames not available yet\n");
            return 0;
        }
#ifdef CUSTOM_CHANGE
        cnt=dump_diff_image(void);
        frame_buffer.save_frame[cur_head_idx].q_empty = TRUE;
#else

        cnt=process_image((void *)&(frame_buffer.save_frame[frame_buffer.head_idx].frame[0]), HRES*VRES*PIXEL_SIZE);
#endif
        frame_buffer.head_idx = (frame_buffer.head_idx + 1) % frame_buffer.ring_size;
        frame_buffer.count = frame_buffer.count - 1; //FIXME: COUNT should be taken care at later service

     	
        printf("rb.tail=%d, rb.head=%d, rb.count=%d ", frame_buffer.tail_idx, frame_buffer.head_idx, frame_buffer.count);
                ...
                else
		{
		    ...
		    if(read_framecnt > 1)
	            {	
                        ...
                        memcpy((void *)&(frame_buffer.save_frame[frame_buffer.tail_idx].frame[0]), buffers[frame_buf.index].start, frame_buf.bytesused);
			printf("memcpy to rb.tail=%d, rb.head=%d, ptr=%p\n", frame_buffer.tail_idx, frame_buffer.head_idx, (void *)&(frame_buffer.save_frame[frame_buffer.tail_idx].frame[0]));
                        ...
                        process_image((void *)&(frame_buffer.save_frame[frame_buffer.head_idx].frame[0]), HRES*VRES*PIXEL_SIZE);
			...
			printf("process from rb.tail=%d, rb.head=%d, ptr=%p\n", frame_buffer.tail_idx, frame_buffer.head_idx, (void *)&(frame_buffer.save_frame[frame_buffer.head_idx].frame[0]));
...
static void init_mmap(char *dev_name)
{
	...
	frame_buffer.tail_idx=0;
	frame_buffer.head_idx=0;
	frame_buffer.count=0;
	frame_buffer.ring_size=3*FRAMES_PER_SEC;
       
         for (i = 0; i < frame_buffer.ring_size; i++) {
              frame_buffer.save_frame[i].q_empty = TRUE;
              frame_buffer.save_frame[i].frame_processed = FALSE;
	 }         
